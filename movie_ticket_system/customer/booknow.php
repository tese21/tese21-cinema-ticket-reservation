...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    error_log("Received SeatID: " . $seatID);
        ...
        if ($stmt->num_rows == 0) {
            ...
            if ($stmt === false) {
                ...
                if ($stmt->num_rows == 0) {
                    ...
                    // Check if the seat ID exists in the available_seat table
                    $stmt->close(); // Close the previous statement
                    $stmt = $conn->prepare("SELECT SeatID FROM available_seat WHERE SeatID = ?");
                    if ($stmt === false) {
                        $error_message = "Error preparing the statement: " . $conn->error;
                    } else {
                        ...
                        $stmt->store_result();
                        if ($stmt->num_rows == 0) {
                            // If the seat ID is not found
                            $error_message = "Invalid SeatID: " . htmlspecialchars($seatID);
                        } else {
                            // If the user ID, movie ID, and seat ID are valid, proceed with the transaction
                            $stmt->close(); // Close the previous statement
                            ...
                            if ($stmt === false) {
                                ...
                                $stmt->bind_param("iisdss", $userID, $payment_method, $movieID, $seatID, $amount, $tx_ref);
                                if (!$stmt->execute()) {
...
// Fetch user ID, movie ID, and seat ID from query parameters
$payment_method = isset($_GET['payment_method']) ? $_GET['payment_method'] : '';
$userID = isset($_SESSION['UserID']) ? $_SESSION['UserID'] : '';
$movieID = isset($_GET['movie_id']) ? $_GET['movie_id'] : '';
$seatID = isset($_GET['seat_id']) ? $_GET['seat_id'] : '';
?>
...
<head>
  ...
  <style>
    ...
    .checkout-container button:hover {
     ...
     <script>
        function bookNow(movieID, movieTitle) {
      const userID = "<?php echo $userID; ?>";
      const seatID = prompt("Please enter your seat ID:");
      const paymentMethod = prompt("Please enter your payment method:");
      const amount = prompt("Please enter the amount:");

      if (seatID && paymentMethod && amount) {
        const url = `../booking/checkout.php?user_id=${encodeURIComponent(userID)}&movie_id=${encodeURIComponent(movieID)}&seat_id=${encodeURIComponent(seatID)}&payment_method=${encodeURIComponent(paymentMethod)}&amount=${encodeURIComponent(amount)}`;
...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
  <div class="checkout-container bg-white p-8 rounded-lg shadow-lg w-full max-w-md animate__animated animate__fadeIn">
    ...
    <form action="booknow.php" method="POST">
      ...
      <input type="hidden" name="seat_id" value="<?php echo htmlspecialchars($seatID); ?>">
      <div class="mb-4">
